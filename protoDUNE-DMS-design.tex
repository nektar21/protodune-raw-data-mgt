...
File Transfer Service (F-FTS or just FTS), which has been successfully
...
\subsection{Fermilab File Transfer System}

Figure~\ref{fig:ftsbasics} shows a cartoon of the basic functionality
of a single FTS instance.  An FTS instance is bound to one or more
sources of input or ``dropboxes''.  A dropbox may be a POSIX-like file
system directory or a source of data accessed through a protocol layer
(\textit{eg} XRootD or gridFTP).
...
(\textit{eg} via the \texttt{mv} command) and otherwise the producer
of a file runs independently from any FTS instance.  Depending on the
nature of the dropbox, multiple file producers may contribute input files.

An FTS instance periodically scans its dropbboxes.  As new files are
discovered, FTS will initiate transfer of the file to its endpoints.
Transfers may be handled directly by FTS or may be delegated to one of
several $3^\mathrm{rd}$ party mechanisms.  As described in
...
they reside in a dropbox.  Throughout these actions (discovery, transfer,
...
\begin{figure}[tbh]
  ...
  \caption{Basic functionality of an FTS instance with a single input dropbox.  An optional opportunistic processing of data while it resides in the dropbox is depicted.}
  \label{fig:ftsbasics}
\end{figure}
...
Two FTS instances will be used to marshal raw data from the protoDUNE
detectors in a manner diagrammed in Fig.~\ref{fig:ftsinstances}.  The
``primary'' or FTS-1 instance is responsible for transferring data
from the disk buffer farms to CERN's central EOS.  The ``secondary''
or FTS-2 instance is responsible for transferring files from EOS to
CASTOR, Fermilab and other receivers of the raw data set.

This tiered approach is employed in order to make prompt use of the
files files (see Section~\ref{sec:prompt}) from the central CERN EOS
disk system.

\begin{figure}[tbh]
  \centering
  \includegraphics[width=0.8\textwidth]{ftsinstances.pdf}
  \caption{The two instances of FTS used for marshaling raw data from the protoDUNE detectors.}
  \label{fig:ftsinstances}
\end{figure}


\subsection{Prompt Use}
\label{sec:prompt}

While raw data files are available on EOS they will be subject to
prompt processing and other access.  Likely scenarios are shown in
Fig.~\ref{fig:prompt}.
...
access to the files while they are on CERN EOS via XRootD.  The data
catalog (Sec.~\ref{sec:sam}) will be used to identify global file
...
layers in the European and North American zones to ensure high speed,
