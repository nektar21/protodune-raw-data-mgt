...
\subsection{Raw Data Management: the Concept}
%%%%%%%%%%%%%%%%%%%%%%%%
\begin{figure}[tbh]
\centering\includegraphics[width=\linewidth]{protoDUNE_raw_data_concept.png}
\caption{\label{fig:raw_concept}Conceptual diagram of the \pd raw data management}
\end{figure}
%%%%%%%%%%%%%%%%%%%%%%%%

Conceptual diagram of the raw data management in \pd is presented in Fig.\ref{fig:raw_concept}. It reflects the central role of the EOS
system at CERN (high-performance distributed disk storage) in the \pd raw data management scheme. This is motivated by the experience
...
File Transfer Service (F-FTS) implemented as two separate systems within the CERN computing environment, which interface each other. They will be referred to
as ``primary'' and ``secondary''. This does not reflect on their scale, prioritization or importance, but rather on their position in the data transmission chain.
