...
Data centers at BNL and NERSC are placed in this diagram for illustration purposes only. Other institutions possessing adequate
...
processing locally at the site of the experiment. To make this possible, design and plans exist to deploy the necessary
computing resources (equivalent to hundreds of cores) in the data room of NP02. A number of options are being explored
for the NP02 buffer storage solution, including GPFS and XRootD.
...
It is important to note that -- as mentioned above -- NP02 and NP04 will use the ``buffer'' in two very different ways.  In NP02
...
(e.g. via the \texttt{mv} command) -- otherwise the producer
...
A FTS instance periodically polls its dropboxes, and as new files are
discovered it will initiate transfer of the file to its endpoints.
...
transfer protocols, the FTS can use these to reduced overhead in performing the actual file transfer operations  (see Fig. \ref{fig:ftsbasicsthird}).
...
In particular the system will natively support the xrootd, gsiftp (gridftp), webdav(http), and srm protocols for access to EOS,
Castor and dCache/Enstore (as appropriate).  The system can support multi-channel transfers and 3$^{rd}$ party transfers
