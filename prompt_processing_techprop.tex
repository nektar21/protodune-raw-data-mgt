...
diagnostics are committed to long term storage (possibly utilizing F-FTS and SAM).

\end{itemize}

\section{Summary}
A design of the Prompt Processing system for \expname is proposed. Variations are possible
while the version which appears optimal is outlined below:
...
adequate throughput and timely delivery of the results to the end user.

\item Payloads are matched to pilots according to priorities. The pilots initiate the execution on the WNs where they run.
...
\item Visual products and other data required by the operators of the experiment is made available via a Web service.
