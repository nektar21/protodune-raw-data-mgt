...
are assumed at the time of writing: the raw data rate
produced by the LArTPC will be $\sim$1.4\,GB/s after lossless compression, and to size up
...
\begin{itemize}
\item The LArTPC stream which is the dominant in terms of rate and volume
\item \textbf{BI} (Beam Instrumentation) Stream containing data from Cherenkov and TOF systems
\item \textbf{CRT} (Cosmic Ray Tagger)
\end{itemize}
...
frature of the \textbf{BI} stream is that the data wil be bundled in pieces potentially as large as coveing
a SPS cycle which is in tens of seconds. It can be assume that the data in all three streams will be tagged
...
position-sensitive  detectors which are part of \textbf{BI}.
...
that the prompt processing system needs to be located on the CERN main campus (the West Area)
as opposed to Prevessin (the North Area).

This argument is bolsered by the fact that EOS will also be the first common point for the data coming
in three streams (\ref{sec:streams}): LArTPC, \textbf{BI} and \textbf{CRT}. The information contained
in the two latter streams will be absolutely necessary for the prompt processing to be meaningful.
